...
export const createStagehandInstance = async (
  ...
  // Default to LOCAL mode unless cloud is explicitly requested
  ...
  if (useCloud && (!apiKey || !projectId)) {
    throw new Error("Browserbase API Key and Project ID are required for cloud mode");
  }

  const modelName = params.modelName || config.modelName || "google/gemini-3-flash-preview";
  const modelApiKey =
    config.modelApiKey ||
  ...
  const stagehand = new Stagehand({
    ...
    // Enable experimental for hybrid mode support
  ...
  if (process.env.VERCEL_AUTOMATION_BYPASS_SECRET) {
    try {
      ...
      if (page) {
        ...
        // Check for CDP session BEFORE enabling Fetch interception
        // Stagehand exposes mainSession (not _cdpSession or cdpSession)
        const cdpSession = (page as any).mainSession;

        if (cdpSession && typeof cdpSession.on === 'function') {
          // Set up the event handler first
          cdpSession.on('Fetch.requestPaused', async (params: any) => {
            try {
              ...
              // If we can't continue with headers, just continue normally
              await page.sendCDP("Fetch.continueRequest", { requestId: params.requestId }).catch(() => {});
            }
          });

          // Now enable Fetch interception only for vercel.app domains
          await page.sendCDP("Fetch.enable", {
            patterns: [
              { urlPattern: "*://*.vercel.app/*", requestStage: "Request" },
            ],
          });

          process.stderr.write(`[SessionManager] Vercel bypass headers configured for *.vercel.app domains (session ${sessionId})\n`);
        } else {
          // Cannot set up interception - do NOT enable Fetch (would cause hangs)
          process.stderr.write(`[SessionManager] WARN - CDP session not available for Fetch interception, Vercel bypass disabled\n`);
        }
      }
    } catch (err) {
      process.stderr.write(`[SessionManager] WARN - Failed to setup Vercel bypass: ${err instanceof Error ? err.message : String(err)}\n`);
    }
  }
...
export class SessionManager {
  ...
  constructor(contextId?: string) {
    ...
    this.defaultSessionId = `stagehand_session_${contextId || "default"}_${Date.now()}_${uniqueId}`;
  ...
  /**
   * Creates a new browser session using Stagehand.
   ...
   * @param cloud - Whether to use Browserbase cloud (default: false, uses local Playwright)
   */
  async createNewBrowserSession(
    ...
    try {
      const modeLabel = useCloud ? "cloud" : "local";
      process.stderr.write(
        `[SessionManager] ${resumeSessionId ? "Resuming" : "Creating"} ${modeLabel} Stagehand session ${newSessionId}...\n`,
      );
      ...
      // For cloud mode, log the Browserbase session info
      ...
      if (useCloud) {
        ...
        process.stderr.write(
          `[SessionManager] Stagehand initialized in LOCAL mode\n`,
        );
      }

      const sessionObj: BrowserSession = {
        page,
        sessionId: browserbaseSessionId || newSessionId, // Use internal ID for local sessions
        stagehand,
      };
